...
namespace SleekFlowTodoListAPI.Infrastructure.Swagger
{
        ...
        public class SecurityRequirementsOperationFilter : IOperationFilter
        {
            ...
            public void Apply(OpenApiOperation operation, OperationFilterContext context)
            {
                ...
                if (context != null && operation != null)
                {
                    ...
                    if ((requiredScopes != null? requiredScopes.Contains(Constants.AzureAdB2C) : false) || requiredScopes2.Contains(Constants.AzureAdB2C))
                    {
                        requireAuth = true;
                        id = Constants.AzureAdB2C;
                        scope = azureAdB2CSwaggerOptions?.Scope ?? "";
                    }
                    else if ((requiredScopes != null ? requiredScopes.Contains(JwtBearerDefaults.AuthenticationScheme) : false) || requiredScopes2.Contains(JwtBearerDefaults.AuthenticationScheme))
                    {
                        requireAuth = true;
                        id = JwtBearerDefaults.AuthenticationScheme;
                    }
